...
import { ButtonGroup, CheckBox, Input } from '@rneui/themed';
		...
		<SafeAreaView style={{ flex: 1 }}>
			<View style={styles.container} >
				...
				<Section heading="Informações pessoais">
					<View style={{ flexDirection: 'column', justifyContent: 'space-between', paddingTop: 10, width: '100%', gap: 10 }}>
						<Input label="Nome" onChangeText={setUsername} onBlur={() => handleProfileFieldBlur('first_name', username)} value={username} style={{ width: '100%', height: '100%' }} />
						<Input label="Apelido" onChangeText={setSurname} onBlur={() => handleProfileFieldBlur('last_name', surname)} value={surname} />
						<Input label="Data de Nascimento" onChangeText={setBirthDate} onBlur={() => handleProfileFieldBlur('date_of_birth', birthDate.toString())} value={birthDate.toString()} />
					</View>
				</Section>
				<Section heading="Dados de contacto">
					<Input label="Email" onChangeText={setEmail} onBlur={() => handleProfileFieldBlur('email', email)} value={email} />
					<Input label="Número de Telemóvel" onChangeText={setPhone} onBlur={() => handleProfileFieldBlur('phone', phone)} value={phone} />
				</Section>
				<Section heading="Perfil de Passageiro">
					{passengerTypes.options.map((item, index) => (
						<CheckBox key={index} checked={passengerProfile === item} checkedIcon="dot-circle-o" onPress={() => { setPassengerProfile(item), handleProfileFieldBlur('work_setting', item) }} title={item} uncheckedIcon="circle-o" />
					))}
				</Section>
				<Section heading="Tipo de utilização">
					{utilizationTypes.options.map((item, index) => (
						<CheckBox key={index} checked={usageType === item} checkedIcon="dot-circle-o" onPress={() => { setUsageType(item), handleProfileFieldBlur('utilization_type', item) }} title={item} uncheckedIcon="circle-o" />
					))}
				</Section>
				<Section heading="Temas de interesse">
					{interests.map((item, index) => (<CheckBox key={index} checked={interestTopics === item} checkedIcon="dot-circle-o" onPress={() => setInterestTopics(item)} title={item} uncheckedIcon="circle-o" />))}
				</Section>
			</View>
		</SafeAreaView>
	);
